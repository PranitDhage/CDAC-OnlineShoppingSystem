...
const CartScreen = (props) => {
  ...
  useEffect(() => {
    ...
    if (cartItems != null) {
      ...
      console.log(cartItems)
      cartItems.map((c) => {
        console.log(total + ' + ' + c.cart_quantity + ' * ' + c.product.prod_price)
        total = total + c.cart_quantity * c.product.prod_price
        setTotal(total)
      })
    }
  }, [cartItems])
  ...
  useEffect(async () => {
    if (
      ...
      cartRemoveStore.response.status == 'OK'
    ) {
      dispatch({
  ...
  const onIncrement = (product) => {
    if (product.cart_quantity < 5) {
      if (product.cart_quantity < product.product.prod_qty) {
        console.log('increased: ')
        console.log(product)
        product.cart_quantity = product.cart_quantity + 1
        setTotal(total + product.product.prod_price)
                          ...
                          <tr>
                            <td>
                              <figure class="itemside">
                                ...
                                <figcaption class="info">
                                  <Link
                                    to={`/productdetails/${c.product.prod_id}`}
                                    class="title text-dark">
                                    {c.product.prod_title}
                                  </Link>
                                </figcaption>
                              </figure>
                            </td>
                            ...
                            <td>
                              <div class="price-wrap">
                                <var class="price">{c.product.prod_price} ₹</var>
                              </div>
                            </td>
                            <td class="text-right">
                              <button
